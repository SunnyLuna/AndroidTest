 ...
 *允许短路 short-circuit ，并且不调用 chain.proceed()。
 * （注：这句话的意思是Chain.proceed()不需要一定要获取来自服务器的响应，但是必须还是需要返回Respond实例。
 * 那么实例从哪里来？答案是缓存。如果本地有缓存，可以从本地缓存中获取响应实例返回给客户端。这就是short-circuit (短路)的意思）
 *允许请求失败重试，并多次调用 chain.proceed();

 *网络拦截器
 *能够对重定向和重试等中间响应进行操作             不适合打印请求日志
 *不允许调用缓存来short-circuit (短路)这个请求。（注：意思就是说不能从缓存池中获取缓存对象返回给客户端，
 * 必须通过请求服务的方式获取响应，也就是Chain.proceed()）
 *观察网络传输中数据传输和变化（注：比如当发生了重定向时，我们就能通过网络拦截器来确定存在重定向的情况）
 *可以获取 Connection 携带的请求信息（即可以通过chain.connection() 获取非空对象）
 *
 * networkInterceptor在CacheInterceptor下级,所以可以修改本次resopnse中的header中缓存相关的参数,
 * 以便于CacheInterceptor来按我们的需求存储本次response
